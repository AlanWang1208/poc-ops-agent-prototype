...
description: 通过平台只读 SQL 助手工具解释 SQL、分析错误、给出优化建议、生成 SELECT 草稿或总结结果对比。
---

# SQL Assistant Advice Read

当操作员需要解释 SQL、分析 SQL 错误、理解校验拒绝原因、获取性能优化建议、从自然语言生成 SELECT 草稿，或总结 SQL 结果对比 diff 时使用本 Skill。
...
- `assistantAction`: `EXPLAIN_SQL`、`OPTIMIZE_SQL`、`ANALYZE_ERROR`、`GENERATE_SELECT` 或 `COMPARE_SUMMARY`。
- `sql`: 需要分析的 SQL 文本。
...
使用返回的 `summary` 作为结论摘要，使用 `suggestions` 给出具体解释、优化建议或 SELECT 草稿。若结果包含 `suggestedSql`，必须明确它只是建议，执行前仍需由服务端重新校验。`COMPARE_SUMMARY` 只能总结平台传入的确定性 diff，不得补充未被 diff 支持的原因或修复动作。使用 `safetyNotes` 说明只读边界、重新校验要求或平台拒绝原因。
