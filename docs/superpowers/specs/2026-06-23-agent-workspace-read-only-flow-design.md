...
- 状态：已批准，待实施
- 设计结论：采用方案 A，让 Agent 工作区直接接入现有 `node-health-read@1.1.0` 只读事件流，先跑通页面到控制面、持久化工作流、独立 Worker、语义事件和结果展示的闭环；不新增 Skill，不启用 AgentRuntime，不引入生产写执行。
...
其中真正可执行的 Worker 适配器是 `node-health-read@1.1.0`。其他已注册 Skill 当前主要用于目录、路由和发布校验展示。另一个 `/api/v1/agent/diagnostics` 入口属于 AgentRuntime 路径，默认关闭，且不适合本次证明 Worker 全链路。
...
- 不启用 `/api/v1/agent/diagnostics` 或 AgentRuntime。
