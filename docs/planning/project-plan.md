...
- 产品边界：公司内部自研自用、共享底座部署，允许内部 Team Workspace 团队级定制，不建设外部 SaaS 租户、计费或外部客户接入能力
...
| Team Workspace 定制闭环 | 进行中 | 15% | 已接受内部 Team Workspace 逻辑租户边界，明确共享库逻辑隔离、工作空间作用域角色、Skill 与策略定制、v2 契约和全局安全基线不可放宽 | 落地身份、策略、Skill 路由、工作流、审计、语义事件和操作台的工作空间上下文 |
...
- 内部 Team Workspace 的最小定制闭环，包括工作空间上下文、工作空间作用域角色、Skill 启用清单和只读诊断隔离
- 评测集、回归检查和 MVP 验收证据
...
- 审批绕过机制
- 高风险执行
- 由人格配置或 Team Workspace 配置降低平台安全要求
...
- M09 已补齐“当前工作流内自动恢复”的第一批实现：
  - 控制面新增只读恢复 SSE 接口；
  - 工作流存储支持按 `workspaceId + workflowId + afterSequence` 查询后续事件；
