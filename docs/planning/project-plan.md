...
| M07 受限执行 Worker | 进行中 | 68% | 已提供独立 WebFlux Worker、回环地址开发配置、显式允许列表和 `node-health-read` 适配器；已新增控制面到 Worker 的 HMAC 传输认证、Worker 入站验签、非回环绑定启动保护、ADR 和运行手册 | 完成 mTLS、受控网络出口、短期目标系统凭据、Windows 隔离部署方案和生产演练 |
