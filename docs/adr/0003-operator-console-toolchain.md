...
- React Markdown 与 Remark GFM：将 Agent 返回的 Markdown 摘要渲染为 React 元素，并支持 GFM 表格、列表、强调和代码等只读展示格式，MIT 许可证。
...
2026-06-28 为 Agent 工作区摘要展示新增 `react-markdown` 与 `remark-gfm`。现有 React/JSDoc/Zod 栈只能安全承载强类型 API 数据和纯文本展示，若自行解析 Markdown 表格、列表和行内格式会快速形成不完整且难维护的解析器。该依赖只用于 M09 前端展示层，不改变后端契约、授权决策、执行事实源或模型能力。模型输出仍按不可信数据处理：操作台不使用 `dangerouslySetInnerHTML`，不启用原始 HTML 解析，并由 React 元素渲染与组件映射限制输出表面。
