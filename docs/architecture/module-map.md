...
- M09 操作台首轮原型化重写当前已经实现：
  - 浏览器会话登录页与受保护路由；
  - Agent 工作台接入只读候选 Skill 路由接口，任务发送保持禁用；
...
- AgentScope Java 主运行时已作为 P1 只读诊断主链路接入：
  - AgentScope Java `1.0.12` 通过 `control-plane-agentruntime` 模块提供 ReAct 主运行循环；
  - `/api/v1/agent/diagnostics` 是 Agent 只读诊断产品主入口，`/internal/agent/diagnostics` 仅作为内部兼容入口保留；
  - 确定性单 Skill 只读入口保留为联调、兼容和紧急回退路径，不再作为产品主链路；
