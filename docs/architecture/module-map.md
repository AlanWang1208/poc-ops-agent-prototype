...
- M04 当前已经实现：
  ...
  - 控制面候选查询接口
- AgentScope Java 主运行时 POC 将作为 M04 的候选实现推进：
  - AgentScope Java 可主导只读诊断意图理解、计划生成和多步 Tool 调用循环；
  - Tool Catalog 必须由 M03 注册与发布校验结果生成，并受 Team Workspace 可见性和 P1 只读风险约束；
  - 每一次 Tool Call 仍必须经过 M02 策略拦截、M05 工作流持久化和 M07 Worker 隔离执行；
